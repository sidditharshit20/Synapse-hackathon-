...
"""
train_scheduler_realtime.py
Unified GA scheduler + live-API adapter + warm-start reoptimizer.

Usage:
    - By default uses a mock live adapter for testing.
    - To use a real API, implement LiveAdapter.fetch_live_updates(...) or
      adapt the provided example.
"""
...
# ----------------------------
# Live API Adapter interface
# ----------------------------
class LiveAdapter:
    """
    Subclass this and implement fetch_live_updates(trains) -> Dict[train_id, update_dict]
    where update_dict can contain fields like 'arrival' (new arrival time in minutes),
    'delay' (minutes), 'cross_time' etc.

    Example return:
      {"T1": {"arrival": 610}, "T3": {"delay": 5}}
    """
    def fetch_live_updates(self, trains: Dict[str, dict]) -> Dict[str, dict]:
...
class MockLiveAdapter(LiveAdapter):
    def __init__(self, scenario_mods: Optional[List[Tuple[int, str, dict]]] = None):
        """
        scenario_mods: list of (after_seconds, train_id, update_dict)
          - after_seconds: when to apply (relative to start)
          - train_id: id to update
          - update_dict: e.g. {"arrival": +10} (adds minutes)
        This allows simulating delays arriving over time.
        """
        self.mods = scenario_mods or []
        self.start_time = time.time()
        self.applied = set()

    def fetch_live_updates(self, trains):
        res = {}
        now = time.time()
        elapsed = now - self.start_time
        for i, (after_s, train_id, upd) in enumerate(self.mods):
            if i in self.applied:
                continue
            if elapsed >= after_s:
                # apply update; for convenience if value is numeric and represents delta, we treat as +delta
                res[train_id] = {}
...
# ----------------------------
# Helper: update trains dict with live updates in-place
# ----------------------------
def apply_live_updates(trains: Dict[str, dict], updates: Dict[str, dict]) -> None:
    for tid, upd in updates.items():
        if tid not in trains:
            continue
        for k, v in upd.items():
            # If update is numeric and keyed as 'arrival' or 'delay', treat as additive
            if k in ("arrival", "delay") and isinstance(v, (int, float)):
                # 'delay' means add to arrival
                if k == "delay":
                    trains[tid]["arrival"] += int(v)
                else:
                    # if arrival provided as absolute (>=1000) or a delta (< 1000?) — we'll assume delta if small
                    # To avoid ambiguity, include exact arrival minute in your API updates if possible.
                    if abs(v) < 1000 and ("+" in str(v) or v < 2000):
                        # interpret as delta if smaller than e.g., 10000
                        trains[tid]["arrival"] += int(v)
                    else:
                        trains[tid]["arrival"] = int(v)
            else:
                # other keys set directly
                trains[tid][k] = v
